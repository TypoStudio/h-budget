...
- 스프레드시트 연결: 구글 피커로 내 드라이브에서 고르거나 새 스프레드시트 생성
...
2. **Google Picker API**(및 Google Drive API) 사용 설정과 **API 키** 발급 — 시트 선택 창에 필요. 키는 `src/App.tsx`의 `PICKER_API_KEY`에 넣고, 콘솔에서 허용 API(Picker·Drive)와 HTTP 리퍼러를 제한해둘 것
3. OAuth 클라이언트의 승인된 JavaScript 원본에 `http://localhost:5173` (배포 시 배포 주소도) 추가
4. OAuth 동의 화면의 요청 범위는 `drive.file` 하나 — 비민감 범위라 '확인되지 않은 앱' 경고 없이 쓸 수 있습니다.
   앱은 피커에서 고른 시트와 앱이 만든 시트에만 접근하며, Sheets API도 이 범위로 동작합니다
